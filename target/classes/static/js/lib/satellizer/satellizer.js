  ...
  angular.module('satellizer', [])
    .constant('satellizer.config', {
      ...
      baseUrl: (function(){
			...
			if(domain === 'localhost' || domain === '106.51.67.211'){
				baseUrl = 'http://'+window.location.hostname + ":" +window.location.port+'/evf-api';
			}else if(domain === 'admin.eveningflavors.com'){
				baseUrl = 'https://www.eveningflavors.com/evf-api';
			}else{
				baseUrl = 'https://dev.eveningflavors.com/evf-api';
			}
			return baseUrl;
      })(),
      loginRedirect: '/callcenter/dashboard',
      logoutRedirect: '/callcenter/login',
      signupRedirect: '/login',
      loginUrl: '/callcenter/agentLogin.json',
      ...
      providers: {
        ...
        facebook: {
          ...
          redirectUri: window.location.origin + '/' || window.location.protocol + '//' + window.location.host + '#/Bangalore',
          scope: ['email'],
          scopeDelimiter: ',',
    ...
    .provider('$auth', ['satellizer.config', function(config) {
      ...
      Object.defineProperties(this, {
        ...
        forgetPasswordUrl: {
            get: function() { return config.forgetPasswordUrl; },
            set: function(value) { config.forgetPasswordUrl = value; }
          },
        loginRoute: {
      ...
      this.$get = [
        ...
        'satellizer.oauth',
        'Account',
        function($q, shared, local, oauth,Account) {
          var $auth = {};
          
          $auth.setloginRedirect = function(value) { 
        	  shared.setloginRedirect(value); 
    	  };
        
         $auth.setlogoutRedirect = function(value) { 
        	 shared.setlogoutRedirect(value); 
          };
          ...
          $auth.forgetPassword = function(user) {
              return local.forgetPassword(user);
            };
          ...
          $auth.isUser = function(){
          	if(Account.getUser('user').role==='User')
          		return true; 
          	else
          		return false;
          }
          $auth.isCorporator = function(){
          	if(Account.getUser('user').role==='Corporator')
          		return true; 
          	else
          		return false;
          }
          $auth.isRestaurateur= function(){
          	if(Account.getUser('user').role==='Restaurateur')
          		return true;
          	else
          		return false;
          }
    ...
    .factory('satellizer.shared', [
      ...
      function($q, $window, $location, config, storage) {
        ...
        shared.setloginRedirect = function(value) { 
      	  config.loginRedirect = value; 
      	  };
      
      	shared.setlogoutRedirect = function(value) { 
        ...
        shared.logout = function(redirect) {
          storage.remove(tokenName);
          storage.remove('callCenterAgent');
    ...
    .factory('satellizer.local', [
      ...
      function($q, $http, $location, utils, shared, config) {
        ...
        local.login = function(user, redirect) {
          ...
          return $http.post(loginUrl, JSON.stringify(user))
            .then(function(response) {
              shared.setToken(response, redirect);
              return response;
            });
        ...
        local.forgetPassword = function(user) {
            var forgetPasswordUrl = config.baseUrl ? utils.joinUrl(config.baseUrl, config.forgetPasswordUrl) : config.forgetPasswordUrl;
            return $http.post(forgetPasswordUrl, user)
              .then(function(response) {
            	  // TODO
                return response;
              });
